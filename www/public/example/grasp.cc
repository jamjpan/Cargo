...
void CargoWeb::construct(const int& i) {
...
void CargoWeb::match() {
  ...
  for (int count = 0; count < max_iter; ++count) {
    ...
    gui::reset();
    print << "Visualizing Sol " << count << " initial" << std::endl;
    for (const auto& kv1 : initial_solution) {
      for (const CustId& cid : kv1.second.first) {
        gui::chi(cid);
        gui::clinev(cid, kv1.first->id());
    ...
    // Visualize improved assignments
    gui::reset();
    print << "Visualizing Sol " << count << " improved" << std::endl;
    for (const auto& kv1 : local_best) {
      for (const CustId& cid : kv1.second.first) {
        gui::chi(cid);
        gui::clinev(cid, kv1.first->id());
        // gui::newroute(kv1.second.first.route());
...
Solution CargoWeb::initialize(Grid& grid, vec_t<Customer>& customers) {
...
Solution CargoWeb::replace(const Solution& solution, vec_t<Customer>& customers) {
...
Solution CargoWeb::swap(const Solution& solution) {
...
Solution CargoWeb::rearrange(const Solution& solution) {
  if (solution.empty()) {
...
Customer CargoWeb::roulette(const dict<Customer, DistInt>& fitness) {
  if (fitness.size() == 0) {
...
DistInt CargoWeb::cost(const Solution& solution) {
  DistInt sum = 0;
...
void CargoWeb::print_sol(const Solution& sol) {
...
void CargoWeb::commit(const Solution& solution) {
