...
auto selector = [](std::pair<VehicleId, Vehicle> pair) {
    return pair.second.route[pair.second.lv_node];
};
...
void NearestNeighbor::UpdateVehicle(const Vehicle &vehicle,
                                    const SimTime time) {
...
//     mqd_t mq = mq_open("/update_vehicle", O_CREAT | O_RDWR, 0655, mq_attr{0,
//     1000000, 1024, 0}); SimTime st; Vehicle v; while (!done_)
//     {
...
void NearestNeighbor::Run() {
    ...
    while (!done_) {
        ...
        if (succeed) {
            ...
            std::transform(vehicles_.begin(), vehicles_.end(),
                           locations.begin(), selector);
            std::transform(vehicles_.begin(), vehicles_.end(), indexes.begin(),
                           selector2);
            ...
            while (!matched && base < n) {
                ...
                std::vector<int> candidates =
                    gtree_.KNN(request.oid, k, locations);
                int i = base;
                while (!matched && i < k) {
                    float distance = 0.0f;
                    int vid;
                    try {
                        ...
                        // v1: originally its lv_stop+1, but that's not correct,
                        // due to the distance from lv_stop to the first in
                        // schedule is quite large v2: back to lv_stop + 1, and
                        // consider the next node in the route as the start
                        std::copy(veh.sched.begin() + lv_stop + 1,
                                  veh.sched.end(),
                                  std::back_inserter(new_sched));
                        if (InsertSchedule(request, veh, new_sched, distance,
                                           new_route)) {
                            matched = true;
                            // concatenate schedule
                            int insert_pos = 0;
                            for (auto iter = veh.sched.begin();
                                 iter != veh.sched.begin() + lv_stop + 1;
                                 ++iter) {
                                new_sched.insert(new_sched.begin() + insert_pos,
                                                 *iter);
                                insert_pos++;
                            }
                            // concatenate route
                            for (int i = lv_node; i >= 0; --i)
                                new_route.insert(new_route.begin(),
                                                 veh.route[i]);
                            // temporarily just make copy to local map
                            vehicles_[veh.id].sched = new_sched;
                            vehicles_[veh.id].route = new_route;

                            if (!sim_->RequestMatched(request, veh.id,
                                                      new_sched, new_route))
                                matched = false;
                        }
...
bool NearestNeighbor::InsertSchedule(const Trip &request,
                                     const Vehicle &vehicle, Schedule &schedule,
                                     float &distance, Route &new_route) {

    // TODO: rethink, the schedule should start from the next node in the route
    // rather than last visited stop
    ...
    // +5 if valid, compute the shortest path from begin to end, set as
    // new_route

    // insert with O(n^2)
    // auto o_iter = schedule.begin();
    // not 0 because the first stop is the last visited stop, so any assigned
    // pickup can't be ahead of it
    int o_iter = 0;
    ...
    // v2: correct_time is the time when the vehicle will visit the next node in
    // the route (round up)
    SimTime correct_time =
        updates_[vehicle.id] - std::ceil((float)vehicle.nnd / speed_);
    ...
    while (o_iter != size) {
        ...
        schedule.insert(schedule.begin() + o_iter,
                        Stop{request.id, request.oid, StopType::CUSTOMER_ORIGIN,
                             -1, request.early});
        ...
        while (d_iter != size + 1) {
            ...
            schedule.insert(schedule.begin() + d_iter,
                            Stop{request.id, request.did,
                                 StopType::CUSTOMER_DEST, -1, request.late});
            // correct the time to the last visited stop, using '-' because nnd
            // is negative
            ...
            // ++iter)
            {
                ...
                if (iter->type == StopType::CUSTOMER_ORIGIN ||
                    iter->type == StopType::VEHICLE_ORIGIN) {
                    // early time window
    ...
    if (found) {
        ...
        // new route. but there may be some problems with the boundray of old
        // route and new route
        gtree_.find_path(schedule_start, schedule[0].node_id, new_route);
        Route temp_route;
        for (int i = 1; i < size + 2; ++i) {
            gtree_.find_path(schedule[i - 1].node_id, schedule[i].node_id,
                             temp_route);
            std::copy(temp_route.begin() + 1, temp_route.end(),
...
int main() {
    ...
    op.ProblemInstancePath = "../../data/benchmark/rs-sm-5.instance";
    ...
    std::cout << "Average: "
              << (sim->TotalMatch() == 0 ? 0
                                         : sim->TotalTime() / sim->TotalMatch())
              << std::endl;
    // std::cout << "Average: " << sim->TotalTime() / sim->TotalMatch() <<
    // std::endl;
    std::cout << "Refused rate: "
              << float(sim->TotalRefuse()) /
                     (sim->TotalMatch() + sim->TotalRefuse()) * 100
              << std::endl;
}
